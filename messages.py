...
def save_message(message, processed=False):
    ...
    save_item(message, 'MC_MESSAGES.' + str(message['update_id']), 3600 * 3)
...
def save_chat_sequence(parent_id, message_id, message_text):
    ...
    save_item(sequence, 'MC_CHAT_SEQUENCE.' + str(parent_id), 3600)
    save_item({'parent_id': parent_id}, 'MC_CHAT_SEQUENCE_INDEX.' + str(message_id), 3600)
